module controller::controller {
  ...
  const EALREADY_MATURED: u64 = 5;
  const EWRONG_MATURITY: u64 = 6;
  const EWRONG_PRICE_RANGE: u64 = 7;
  const EWRONG_TOKEN_PAIRS: u64 = 8;
  const EIS_DISABLED: u64 = 9;
  ...
  public entry fun settle_options(receiver: &signer, token_name: String, amount: u64) acquires ModuleData {
    ...
    if (is_call) {
      ...
      if (strike_price >= last_price) { // ITM, ATM for Put Options
        if (is_plus) {
          coin::transfer<T>(&resource_signer, receiver_addr, amount * (strike_price - last_price));
        } else {
          coin::transfer<T>(&resource_signer, receiver_addr, amount * last_price);
        };
  ...
  public entry fun close_options(receiver: &signer, plus_token_name: String, minus_token_name: String, amount: u64) acquires ModuleData {
    let receiver_addr = signer::address_of(receiver);
    let module_data = borrow_global_mut<ModuleData>(@controller);
    let resource_signer = account::create_signer_with_capability(&module_data.signer_cap);
  ...
  public entry fun set_paused(caller: &signer, paused: bool) acquires ModuleData {
    let caller_address = signer::address_of(caller);
    assert!(caller_address == ADMIN_ADDRESS, error::permission_denied(ENOT_AUTHORIZED));
  ...
  public fun get_payout(receiver_addr:address, token_name: String, balance: u64): u64 {  
    ...
    let last_price = property_map::read_u64(&property, &string::utf8(b"last_price_at_maturity")) * SUFFIX_PRICE_PARSING;
    ...
    if (is_call) {
      ...
      if (strike_price >= last_price) { // ITM, ATM for Put Options
        if (is_long) {
          payout = balance * (strike_price - last_price);
        } else {
          payout = balance * last_price;
        };
      } else { // OTM for Put Options
        if (is_long) {
          payout = 0;
